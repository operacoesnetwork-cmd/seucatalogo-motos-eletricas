...
class Media {
    ...
    onResize(
        ...
        // Reduce size on mobile (adjust multipliers)
        const mobileScaleFactor = isMobile ? 0.6 : 1.0;

        this.plane.scale.y =
            (this.viewport.height * (900 * this.scale * mobileScaleFactor)) / this.screen.height;
        this.plane.scale.x =
            (this.viewport.width * (700 * this.scale * mobileScaleFactor)) / this.screen.width;
...
class App {
    ...
    onResize() {
        ...
        const isMobile = this.screen.width < 768;
        const newBend = isMobile ? 0 : this.initialBend;

        if (this.medias) {
            this.medias.forEach((media) =>
                media.onResize({ screen: this.screen, viewport: this.viewport, bend: newBend }),
            );
        }
    }
    ...
    update() {
        ...
        this.scroll.target += this.scrollSpeed * 0.05; // Force auto-movement
